...
class SystemInitMessage(StreamMessage):
    ...
    def __init__(self, session_id: str, cwd: str = "", tools: List[str] = None, 
                 mcp_servers: List[str] = None, model: str = ""):
...
class AssistantMessage(StreamMessage):
    ...
    def __init__(self, session_id: str, message: Dict[str, Any] = None, 
...
class StreamingJSONHandler:
    ...
    def send_system_init(self, cwd: str, tools: List[str], mcp_servers: List[str], model: str):
    ...
    def send_assistant_tool_use(self, tool_name: str, tool_input: Dict[str, Any], 
                               tool_use_id: Optional[str] = None, message_id: Optional[str] = None):
    ...
    def send_tool_execution_updates(self, tool_calls: List[Dict[str, Any]], tool_results: List[str]):
    ...
    async def process_streaming_response(self, response_generator):
        ...
        async for chunk in response_generator:
            if isinstance(chunk, str):
                ...
                if not any(marker in chunk for marker in ['<tool_call>', '</tool_call>', 'tool_calls']):
...
def main():
    ...
    # Send tool result
    handler.send_tool_result(tool_id, "total 10\ndrwxr-xr-x 2 user user 4096 Jan 1 12:00 .")
