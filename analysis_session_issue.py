...
def analyze_session_flow():
    ...
    print(
        "2. `messages = []` or `messages = session_manager.get_messages()` (for resume)"
    )
    ...
    print()
    print(
        "The issue is that the `messages` list is passed BY REFERENCE to interactive_chat."
    )
    ...
    print(
        "- Line 1659: messages.append({'role': 'assistant', 'content': full_response})"
    )
    print()
    ...
    print(
        "4. session_manager.add_message() is only called AFTER interactive_chat returns"
    )
