...
class ContentProcessor:
    ...
    @staticmethod
    def split_content_and_tools(content: str, tool_patterns: List[str]) -> tuple[str, List[str]]:
        """Split content into text part and tool call parts."""
        text_part = ContentProcessor.extract_text_before_patterns(content, tool_patterns)
...
class ContentProcessorFactory:
    ...
    def extract_text_before_tool_calls(cls, content: str, llm_type: str) -> str:
        ...
        if llm_type.lower() == "deepseek":
            ...
            return ContentProcessor.extract_text_before_patterns(content, generic_patterns)
    
    @classmethod
    def split_content_and_tool_calls(cls, content: str, llm_type: str) -> tuple[str, List[str]]:
...
def split_response_content(content: str, llm_type: str = "generic") -> tuple[str, List[str]]:
