...
class TestBaseMCPAgent:
    ...
    def test_init_main_agent(self, sample_host_config):
        ...
        class TestAgent(BaseMCPAgent):
            ...
            async def _generate_completion(self, messages, tools=None, stream=True, interactive=True):
    ...
    async def test_generate_response_streaming(self, mock_base_agent):
        ...
        result = await mock_base_agent.generate_response([{"role": "user", "content": "test"}])
    ...
    async def test_generate_response_non_streaming(self, mock_base_agent):
        ...
        mock_base_agent._generate_completion = AsyncMock(return_value="Complete response")
    ...
    async def test_execute_builtin_tool_bash(self, mock_base_agent):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            result = await mock_base_agent._execute_builtin_tool("bash_execute", {"command": "echo test"})
    ...
    async def test_compact_conversation(self, mock_base_agent, sample_messages):
        ...
        mock_base_agent.get_token_limit = MagicMock(return_value=100)  # Low limit to force compacting
    ...
    async def test_subagent_task_spawning(self, mock_base_agent):
        ...
        mock_base_agent.subagent_manager.spawn_subagent = AsyncMock(return_value="task_123")
    ...
    async def test_error_handling_in_tool_execution(self, mock_base_agent):
        ...
        result = await mock_base_agent._execute_builtin_tool("bash_execute", {"command": "invalid_command_xyz"})
        assert "error" in result.lower() or "command not found" in result.lower()
        
        # Test with missing file
        result = await mock_base_agent._execute_builtin_tool("read_file", {"file_path": "/nonexistent/file.txt"})
